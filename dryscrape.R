# Dry Scrape
# Last edited 2-28-2016 / Mise-a-jour 2-28-2016
# Manny
...
todays.games <- function(date) { # 2016-02-29
  url <- paste("https://www.nhl.com/schedule/", date, "/ET", sep = "")
  ...
  # "/gamecenter/2015020941\" class=\"icon-label-link\" title=\"GAMECENTER\"
  # <div class=\"section-subheader\">Tuesday, Mar 1</div>
  
  datesubs <- unique(unlist(regmatches(glist, gregexpr("section-subheader\\\">[a-zA-Z0-9, ]+</div>", glist))))
  cutoff <- datesubs[2]
  
  sublist <- unlist(regmatches(glist, gregexpr(paste(".+", cutoff, sep = ""), glist)))
  
  gameids <- gsub("/gamecenter/|\\\" class=\\\"icon-label-link\\\" title=\\\"GAMECENTER", "", unique(unlist(regmatches(sublist, gregexpr("/gamecenter/[0-9]+\\\" class=\\\"icon-label-link\\\" title=\\\"GAMECENTER", sublist)))))
  ids <- sort(substr(gameids, start = 6, stop = 10))
  return(ids)
}

scrape <- function(season, start, end, names, pause) {
  ...
  for (j in 1:((end - start) + 1)) {
    ...
    print(paste("Pausing", pause, "seconds..."))
    Sys.sleep(pause)
  ...
  if (length(missing) > 0) {
    print(paste("Oops! Games missing:", paste(missing, collapse = ", ")))
  } else {
    print("Success! All games were scraped.")
  }
...
bios <- function(teams) {
  ...
  if (length(not.teams) > 0) {
    print(paste("Oops! The following are not properly formatted team names:", paste(not.teams, collapse = ", ")))
  } else {
    print("Success!")
  }
...
quick.scrape <- function(season, games, names, pause) {
...
# Test function / Tester la fonction
start = Sys.time()

scrape(season = "20092010", start = 20020, end = 20020, names = TRUE, pause = 1)
