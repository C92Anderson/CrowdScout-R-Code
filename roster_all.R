...
non_nhl_players <- data.frame(matrix(c("MATTIASOHLUND","NATHANHORTON","CHRISPRONGER","JESSEWINCHESTER","ERICBOULTON","STEPHANEROBIDAS"),ncol = 1))
...
####store current time less 3 hours (PST to EST) & 1 minute
cut.off  <- Sys.time() - (60*60*3) - 60

###loop through and insert
for(i in 1:nrow(hockey_roster)) { 

  query <- paste0("INSERT INTO nhl_all.hockey_roster_R 
                      (`pnm`, `tm_nm`, `no`, `player_name`, `pos`, `height`, `weight`, `dob`, `age`, `pob`, `team`, `nhl_id`)
                                 VALUES('",hockey_roster[i,1],"','",hockey_roster[i,2],"','",hockey_roster[i,3],"','",hockey_roster[i,4] , 
                                  "','", hockey_roster[i,5], "','",
                  gsub("'",".",hockey_roster[i,6]),"','", hockey_roster[i,7],"','", hockey_roster[i,8], "','", hockey_roster[i,9], "','", 
                  gsub("'",".",hockey_roster[i,10]),"','",hockey_roster[i,11], "','", hockey_roster[i,12],"')")
  
dbGetQuery(conn, query)

}

query <- paste0("DELETE FROM `nhl_all`.`hockey_roster_R` WHERE cron_ts is null or cron_ts = 0 or cron_ts < '",cut.off,"'")
dbGetQuery(conn, query)
